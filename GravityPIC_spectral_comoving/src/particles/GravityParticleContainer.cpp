...
void GravityParticleContainer::InitParticles (const std::string& filename) {
...
void
GravityParticleContainer::DepositCharge (const Vector<MultiFab*>& rho) {

    int num_levels = m_gdb->finestLevel() + 1;   // was: rho.size()
    int finest_level = num_levels - 1;
    
    // Each AMR level deposits its own particles onto its own rho MultiFab
    const int ng = rho[0]->nGrow();
    amrex::Print() << "num_levels=" << num_levels << " ng=" << ng << std::endl;
    for (int lev = 0; lev < num_levels; ++lev) {
    amrex::Print() << "lev=" << lev
                    << " rho[lev]=" << rho[lev]
                    << " nGrow=" << (rho[lev] ? rho[lev]->nGrow() : -1)
                    << " nBoxes=" << (rho[lev] ? rho[lev]->boxArray().size() : -1)
                    << std::endl;
    }
    for (int lev = 0; lev < num_levels; ++lev) {
        ...
        for (MyParIter pti(*this, lev); pti.isValid(); ++pti) {
            ...
            amrex::ParallelFor(np, [=] AMREX_GPU_DEVICE (int i) noexcept {
                          deposit_cic(p_ptr[i], m, rhoarr, plo, dxi);
                                   });
        }
    ...
    // Rescale the deposited mass density by 4piG for poisson solve 
    for (int lev = 0; lev < num_levels; ++lev) {
...
// velocity update 
void GravityParticleContainer
::Evolve (const Vector<std::array<const MultiFab*, AMREX_SPACEDIM> >& E,
          const Vector<const MultiFab*>& rho, const amrex::Real kick_factor, const amrex::Real drift_factor) {
        ...
        for (MyParIter pti(*this, lev); pti.isValid(); ++pti) {
            ...
            amrex::ParallelFor(np, [=] AMREX_GPU_DEVICE (int i) noexcept {
                                       push_leapfrog(p_ptr[i].pos(0), p_ptr[i].pos(1), p_ptr[i].pos(2),
                                                     vxp[i], vyp[i], vzp[i],
                                                     Exp[i], Eyp[i], Ezp[i],
                                                     kick_factor, drift_factor, plo, phi);
                                   });
        }
    }
}
// position update
void GravityParticleContainer::pushX (const Real& drift_factor) {
        ...
        for (MyParIter pti(*this, lev); pti.isValid(); ++pti) {
            ...
            amrex::ParallelFor(np, [=] AMREX_GPU_DEVICE (int i) noexcept {
                                       push_leapfrog_positions(p_ptr[i].pos(0), p_ptr[i].pos(1), p_ptr[i].pos(2),
                                                               vxp[i], vyp[i], vzp[i], drift_factor, plo, phi);
                                   });
